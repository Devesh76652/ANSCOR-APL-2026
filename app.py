...
def generate_match_pdf(m):
    ...
    try:
        ...
        if d1["b1"]["name"]:
            ...
            pdf.set_xy(15, y + 2)
            pdf.cell(0, 5, f"INNINGS 1: {m['team_1']}", ln=True)
            pdf.set_text_color(0, 0, 0)
            y += 12
            ...
            if d1["b1"]["name"]:
                ...
                pdf.cell(55, 6, self._clean_text(d1["b1"]["name"][:22]), 1)
            ...
            if d1["b2"]["name"]:
                ...
                pdf.cell(55, 6, self._clean_text(d1["b2"]["name"][:22]), 1)
            ...
            for b in d1.get("all_batsmen", []):
                if b.get("name"):
                    ...
                    pdf.cell(55, 6, self._clean_text(b["name"][:22]), 1)
                    ...
                    pdf.cell(50, 6, self._clean_text(status[:18]), 1, 1, "C")
            ...
            if d1["bowler"]["name"]:
                ...
                pdf.cell(55, 6, self._clean_text(d1["bowler"]["name"][:22]), 1)
        ...
        if d2["b1"]["name"]:
            ...
            pdf.rect(10, y, 190, 8, 'F')
            pdf.set_xy(15, y + 2)
            pdf.cell(0, 5, f"INNINGS 2: {m['team_2']}", ln=True)
            pdf.set_text_color(0, 0, 0)
            y += 12
            ...
            if d2["b1"]["name"]:
                ...
                pdf.cell(55, 6, self._clean_text(d2["b1"]["name"][:22]), 1)
            ...
            if d2["b2"]["name"]:
                ...
                pdf.cell(55, 6, self._clean_text(d2["b2"]["name"][:22]), 1)
        ...
        try:
            ...
            pdf.cell(0, 10, f"Match: {m['team_1']} vs {m['team_2']}", ln=True, align="C")
...
# Add the clean_text method to the function
def clean_text(text):
    import re
    emoji_pattern = re.compile("["
...
# Update the generate_match_pdf to use the clean_text function
def generate_match_pdf(m):
    """Generate a simple PDF scorecard without emojis"""
    try:
        m = ensure_match(m)
            ...
            if is_admin:
                ...
                with col_right:
                    ...
                    if inn["awaiting_batsman"]:
                        st.warning("New Batsman Required")
                    ...
                    if has_data:
                        ...
                        if pdf_data and len(pdf_data) > 500:
                            ...
                            st.warning("Preparing PDF... Please wait a moment then try again.")
